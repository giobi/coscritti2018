---
layout: post
title: "Colloqui 14 Aprile - Prenota il tuo slot"
date: 2026-03-30
category: Scuola
excerpt: "Prenota il colloquio genitori-insegnanti del 14 aprile. Scegli la fascia oraria."
...
async function loadDisponibilita() {
    try {
        const response = await fetch(`${API_URL}/disponibilita?date=${EVENT_DATE}`);
...
document.getElementById('colloquiForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`${API_URL}/prenota`, {
            ...
            body: JSON.stringify({ cognome: cognome, fascia: selectedFascia, date: EVENT_DATE })
        });
        const data = await response.json();
...
async function deletePrenotazione(fascia, cognome) {
    ...
    try {
        const response = await fetch(`${API_URL}/elimina`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ fascia: fascia, cognome: cognome, date: EVENT_DATE })
        });
        const data = await response.json();
